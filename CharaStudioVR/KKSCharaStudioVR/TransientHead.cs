...
namespace KKSCharaStudioVR
{
	public class TransientHead : ProtectedBehaviour
	{
		...
		public void Reinitialize()
		{
			...
			Renderer[] array = (m_tongues = (from renderer in root.GetComponentsInChildren<SkinnedMeshRenderer>()
		...
		private void SetVisibility(bool visible)
		{
			if (visible)
			{
				if (hidden)
				{
					foreach (Renderer renderer3 in rendererList)
					{
						if ((bool)renderer3)
						{
					...
					foreach (Renderer renderer2 in tongues)
					{
						if ((bool)renderer2)
						{
			...
			{
				Renderer[] tongues = (m_tongues = (from renderer in root.GetComponentsInChildren<SkinnedMeshRenderer>()
					where renderer.name.StartsWith("cm_o_tang") || renderer.name == "cf_o_tang"
					select renderer into tongue
					where tongue.enabled
					select tongue).ToArray());
				rendererList.Clear();
				foreach (Renderer item in from renderer in headTransform.GetComponentsInChildren<Renderer>()
