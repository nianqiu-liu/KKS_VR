...
namespace KKSCharaStudioVR
{
	public class KKSCharaStudioActor : DefaultActorBehaviour<ChaControl>
	{
		...
		protected override void OnLateUpdate()
		{
			...
			if ((bool)transform)
			{
				if ((bool)eyeLookCtrl && eyeLookCtrl.target == transform)
				{
					eyeLookCtrl.target = _TargetController.Target;
				}
				if ((bool)neckLookCtrl && neckLookCtrl.target == transform)
				{
					neckLookCtrl.target = _TargetController.Target;
				}
			}
			if (!(base.Actor.asVoice != null))
			{
				return;
			}
			try
			{
				AudioSource asVoice = base.Actor.asVoice;
				asVoice.gameObject.transform.position = base.Actor.objHeadBone.transform.position;
				float minVoiceDistance = (VR.Settings as KKSCharaStudioVRSettings).MinVoiceDistance;
				float maxVoiceDistance = (VR.Settings as KKSCharaStudioVRSettings).MaxVoiceDistance;
				if (asVoice.minDistance != minVoiceDistance || asVoice.maxDistance != maxVoiceDistance)
				{
					VRLog.Debug($"Modify audio parameter {asVoice.name}: ({asVoice.minDistance}, {asVoice.maxDistance}, {asVoice.rolloffMode}) -> ({minVoiceDistance}, {maxVoiceDistance}, {AudioRolloffMode.Logarithmic})");
		...
		internal void OnVRModeChanged(bool newMode)
		{
			...
			if ((bool)transform)
			{
				if ((bool)eyeLookCtrl && eyeLookCtrl.target == _TargetController.Target)
				{
					eyeLookCtrl.target = transform;
				}
				if ((bool)neckLookCtrl && neckLookCtrl.target == _TargetController.Target)
				{
